...
int main(int argc, char ** argv) {
    ...
    matrixMultiply<<<gridsz, blocksz>>>(deviceA, deviceB, deviceC, 
    ...
    wbCheck(cudaMemcpy(hostA, deviceA, (sizeof(float)*numARows*numAColumns), cudaMemcpyDeviceToHost));
    wbCheck(cudaMemcpy(hostB, deviceB, (sizeof(float)*numBRows*numBColumns), cudaMemcpyDeviceToHost));
...
/*
	...
	for (int j = 0; j < numCColumns; ++j) {
		if (j) printf(" ");
		printf("%.7f", hostC[i*numCColumns+j]);
	}
	printf("\n");
    }
 */
